...
@Component({
  selector: 'app-grafico-turnos-solicitado',
  templateUrl: './grafico-turnos-solicitado.component.html',
  styleUrls: ['./grafico-turnos-solicitado.component.css']
})
export class GraficoTurnosSolicitadoComponent implements OnInit {
    ...
    if (this.fechaHasta >= this.fechaDesde) {
      ...
      this.especialistas.forEach(especialista => {
        ...
        this.turnos.forEach(turno => {
          if (this.verificarTurnoEntreFechas(turno, arrayFechas) && turno.especialista.uid == especialista.uid) {
            contador++;
          }
        })
  ...
  async crearPdf() {
    let doc = new jsPDF();
    doc.setFontSize(30);
    doc.text('Cantidad de turnos solicitados', 40, 35);
    ...
    const grafico: any = document.getElementById('graficoSolicitado');
    const options = {
    ...
    doc.output('dataurlnewwindow', { filename: 'turnos-solicitados' });
  }
}
